    ...
    public:
        RoleStation()
        {
            do
            {
                ...
                x=(rand()%20)*50;  // rand()%20= 0 TO 19  X  50
                y=250+(rand()%11)*50;
...
class Passenger { 
    ...
    public:
        ...
        int get5X() { return x-40; }
        int get5Y() { return y-40; }      
};
...
public:
    ...
    void draw() 
    {
        ...
        {  // Draw Buildings
            ...
            if(overGame)
            {
                if(!isMenu && (TotalTime <= 0 || fuel <= 0 || score < 0)) 
                {
...
void NonPrintableKeys(int key, int x, int y) 
{
    ...
    if(ismove)
    {
        for(int i=0;i<2;i++)
        {
            if(checkCollision(x,y,50,50,passenger[i].get5X(),passenger[i].get5Y(),50,50)) 
            {
                ismove=false;
                sounds.playCollision();
                if(ispassenger)
